...
def main():
    ...
    # print(network.modeling.__dict__['deeplabv3plus_mobilenet'](num_classes=2, output_stride=16))
    # print(network.modeling.__dict__[opts.model])(num_classes=2, output_stride=16)
    ...
    model = network.modeling.__dict__['deeplabv3_resnet50'](num_classes=opts.num_classes, output_stride=opts.output_stride, pretrained_backbone=False)
    ...
    example = torch.rand(1, 3, 1440, 1080)
    traced_script_module = torch.jit.trace(model, example)          
    traced_script_module_optimized = optimize_for_mobile(traced_script_module)      
    traced_script_module_optimized._save_for_lite_interpreter("deeplab_model.ptl")  # 해당 코드 실행하면 app/src/main/assets dir에 model.ptl 생성됨
